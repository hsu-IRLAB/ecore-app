...
@HiltViewModel
class UserDailyViewModel @Inject constructor(
    private val prefs: Prefs,
    private val userDailyUseCase: CommonUseCase
): ViewModel() {
    private val _userDaily = MutableLiveData<List<DomainImages>>()
    val userDaily: LiveData<List<DomainImages>>
        get() = _userDaily
    ...
    fun getWholeRanking(){
        viewModelScope.launch {
            _userDaily.value = userDailyUseCase.getDailyProfileImg(prefs.user_id)
        }
    }
}