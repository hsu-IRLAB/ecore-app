...
class FollowingRankingViewModel(private val repository: RankingRepositoryImpl): ViewModel() {
    ...
    init { // 초기화 시 서버에서 데이터를 받아온다.
        viewModelScope.launch {
            _retrofitRanking.value = repository.getRanking("following")
        }
    }
    class Factory() : ViewModelProvider.Factory { // factory pattern
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return RankingRepositoryImpl.getInstance()?.let { FollowingRankingViewModel(it) } as T
        }
    }
